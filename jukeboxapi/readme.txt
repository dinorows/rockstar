...
Dockerized:
docker build --pull -t dinonu/rockstarapi10 .
docker build --platform linux/amd64 -t dinonu/rockstarapi10amd64 .
...
message NETSDK1057: You are using a preview version of .NET. See: https://aka.ms/dotnet-support-policy [/source/rockstarwebapi.csproj]
...
You may also need to include 'linux-arm64' in your project's RuntimeIdentifiers. [/source/rockstarwebapi.csproj]
------
Dockerfile:12
--------------------
  10 |     # Copy source code and publish app
  11 |     COPY --link rockstarapi/. .
...
docker login
docker push dinonu/rockstarapi10
docker push dinonu/rockstarapi10amd64

test:
docker run --rm -it -p 8000:8080 -e ASPNETCORE_HTTP_PORTS=8080 dinonu/rockstarapi10